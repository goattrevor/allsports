...
//
// 가중치 (홈 어드밴티지 0.10은 기본 54%에 반영, 나머지 합 0.90):
//   선발투수 우위  0.30  — 시즌 ERA/WHIP 비교
//   불펜 상태      0.18  — 최근 3일 투구량·연투 기반 피로도
//   최근 폼        0.18  — 최근 10경기 승률
//   타선           0.13  — 시즌 팀 OPS
//   상대전적       0.08  — 올 시즌 맞대결 결과
//   날씨           0.03  — 악천후 시 홈팀 미세 우위 (돔구장은 중립)

import { getBullpenStatus } from './mlbBullpen'

const WEIGHTS = { starter: 0.30, bullpen: 0.18, form: 0.18, offense: 0.13, h2h: 0.08, weather: 0.03 }
...
// 올 시즌 맞대결: 홈팀 기준 { wins, losses }
async function getH2H(homeTeamId, awayTeamId, dateStr) {
  const cache = getCache()
  const key = `h2h:${homeTeamId}:${awayTeamId}:${dateStr}`
  const hit = cache.get(key)
  if (hit && Date.now() - hit.ts < CACHE_TTL_MS) return hit.data

  const season = dateStr.slice(0, 4)
  const data = await fetchJson(
    `https://statsapi.mlb.com/api/v1/schedule?sportId=1&teamId=${homeTeamId}` +
    `&opponentId=${awayTeamId}&startDate=${season}-03-01&endDate=${dateStr}&gameType=R`)
  let wins = 0, losses = 0
  for (const d of data.dates || []) {
    for (const g of d.games || []) {
      if (g.status?.abstractGameState !== 'Final') continue
      const homeSide = g.teams?.home?.team?.id === Number(homeTeamId) ? g.teams.home : g.teams?.away
      if (homeSide?.isWinner) wins++
      else losses++
    }
  }
  const out = { wins, losses, total: wins + losses }
  cache.set(key, { ts: Date.now(), data: out })
  return out
}

// 구장 날씨 (Open-Meteo, 무료·키 불필요). 돔구장은 중립 처리.
async function getWeather(venueId, dateStr) {
  if (!venueId) return null
  const cache = getCache()
  const key = `weather:${venueId}:${dateStr}`
  const hit = cache.get(key)
  if (hit && Date.now() - hit.ts < CACHE_TTL_MS) return hit.data

  const venueData = await fetchJson(
  ...
  const roof = venue?.fieldInfo?.roofType || ''
  const isDome = /dome|retractable/i.test(roof)

  let out = { venue: venue?.name || null, isDome, temp: null, rainPct: null, windKmh: null }
  if (!isDome && coords?.latitude != null) {
    try {
      ...
      const d = wx.daily
      out.temp = d?.temperature_2m_max?.[0] ?? null
      out.rainPct = d?.precipitation_probability_max?.[0] ?? null
      out.windKmh = d?.wind_speed_10m_max?.[0] ?? null
    } catch { /* 날씨 실패는 중립 처리 */ }
  }
  cache.set(key, { ts: Date.now(), data: out })
  return out
...
export async function analyzeGame({ awayTeamId, homeTeamId, awayPitcherId, homePitcherId, venueId, dateStr }) {
  const season = dateStr.slice(0, 4)

  const [starters, standings, awayOPS, homeOPS, awayBp, homeBp, h2h, weather] = await Promise.all([
    ...
    getH2H(homeTeamId, awayTeamId, dateStr).catch(() => null),
    getWeather(venueId, dateStr).catch(() => null),
  ])
  ...
  // 상대전적: 올 시즌 맞대결 승차 (3경기 이상일 때만 의미 부여)
  edges.h2h = h2h && h2h.total >= 3
    ? clamp((h2h.wins - h2h.losses) / h2h.total, -1, 1)
    : 0

  // 날씨: 악천후일수록 홈팀 미세 우위 (원정팀이 환경에 덜 익숙)
  // 돔구장·데이터 없음 = 중립
  if (weather && !weather.isDome && (weather.rainPct != null || weather.windKmh != null)) {
    const rainBad = (weather.rainPct ?? 0) / 100                    // 0~1
    const windBad = clamp(((weather.windKmh ?? 0) - 20) / 30, 0, 1) // 20km/h 이상부터
    edges.weather = clamp(rainBad * 0.6 + windBad * 0.4, 0, 1)
  } else {
    edges.weather = 0
  }
  ...
  const x = WEIGHTS.starter * edges.starter + WEIGHTS.bullpen * edges.bullpen +
            ...
            WEIGHTS.h2h * edges.h2h + WEIGHTS.weather * edges.weather
  ...
  return {
    ...
    facts: {
      ...
      awayForm, homeForm,
      awayOPS, homeOPS,
      h2h,      // 홈팀 기준 { wins, losses, total }
      weather,  // { venue, isDome, temp, rainPct, windKmh }
